...
while cap.isOpened():
    ...
    if lm != None:
        ...
        if offset < 100:
            ...
            cv2.putText(img, str(int(offset)) + 'Tidak Sejajar', (w - 150, 30), font, 0.9, red, 2)
        ...
        # Pose time.
        time_string_good = 'Waktu postur baik: ' + str(round(good_time, 1)) + 's'
        cv2.putText(img, time_string_good, (10, h - 45), font, 0.9, green, 2)
        time_string_bad = 'Waktu postur buruk: ' + str(round(bad_time, 1)) + 's'
        cv2.putText(img, time_string_bad, (10, h - 15), font, 0.9, red, 2)

        time_string_total = 'Total Waktu: ' + str(round(total_time, 1)) + 's'
        cv2.putText(img, time_string_total, (10, h - 75), font, 0.9, blue, 2)

        percent_string = '% Postur baik: ' +str(round(correct_percent, 2)) + ' %'
